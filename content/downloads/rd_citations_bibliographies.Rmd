...
And then just manually enter a bibliography at the end of the 
document:
...
>   Meinshausen, Malte, and Riahi, Keywan (2013). 
...
the ability to automatically format bibliographic information and
manage citations. Using this is a little complicated because you need
to have file with your bibliographic information entered in a certain
format.

Everything that follows describes how to do citations and 
bibliographies with Quarto using bibliography files. If you wish, you
may ignore the rest of this document and manage your citations and
bibliographies manually, as described at the top of this file.

Quarto can work with a wide variety of file formats for
bibliographies, including those produced by many common software
packages, such as EndNote, Zotero, and Mendeley.
Quarto integrates especially well with Zotero.
You can find further documentation about bibliographies and citations
at <https://quarto.org/docs/visual-editor/technical.html#citations>

# Bibliography Files

Quarto reads bibliography entries from an external bibliography file.
To specify the bibloography file, you would just add a line to the
header of your Quarto document.
You can optionally add an additional line `link_citations: true` to
create hyperlinks from citations in the text of your document to the
corresponding bibliography entry. Here's an example:
...
Quarto claims to support the following file types as well, but I 
...
* **Using the Insert Citation tool:** 
  You can bring up the Insert Citation tool by clicking on the "Insert" button 
  on the toolbar, or pressing $\Uparrow \hat{} \text{F8}$ (`Ctrl-Shift-F8`) on 
  your keyboard. This will bring up a dialog box where you can look up references
  in your personal bibliography (including Zotero), via a 
  DOI (digital object identifier) code, or from public bibliographic databases
  such as CrossRef or PubMed.

If you open a BibTeX bibliography file in RStudio, you will see that all references
