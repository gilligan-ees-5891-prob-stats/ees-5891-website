...
slug: intro-repro-research
pdf_url: /files/handouts/intro-repro-research.pdf
output:
  blogdown::html_page:
...
week of Sept. 25--Oct. 2 [@clough_covid_2020]. This caused the service to fail 
to warn people in close 
...
clinical medical research. One recent review of more than 5,000 papers in eight 
top medical journals found that almost 16% had indications of fraud or 
incompetent statistical analyses. [@carlisle_fabrication_2017]
...
For homework and the semester project in EES 4891 and 5891, 
we will be using R scripts to analyze geoscience data.
In the course of the semester, you will become increasingly familiar with the 
ideas of using analysis scripts to promote reproducible research.
...
In EES 4891 and 5891, we will use a tool called Quarto, which
...
* Computational reproducibility provides detailed information on exactly how computations
    ...
    software, so it will be trivial to add this to the end of your 
...
This course is about how to use statistical methods to analyze data that you
already have in hand, 
so I will focus entirely on _computational reproducibility_ 
and will not treat _empirical reproducibility_.
I will not address all the details of computational reproducibility, 
but we will focus on three important aspects:
...
* **Literate computing and authoring:** 
  ...
  In this course, we will use Quarto and RStudio to apply literate
  computing and authoring in homework assignments and writing reports from your
  semester project.
...
* **Automation:** 
  ...
  click on the same menu entries over and over again on a dozen different data 
  sets. Automating your analysis with scripts makes it easy to run the same 
  script on each of your different data sets.
...
* **Revision Control:**
  ...
  Another important use of revision control applies to research projects. 
  I have computational models that are
  ...
  For this course, we will be using the software tool `git` and the web service
  GitHub for revision control for homework assignments and the semester research
  project.
  
## Optional Tools

### Making PDF files from Quarto

If you want to produce PDF files from Quarto, you will need to install the `tinytex` package in R.
...
Since the early 2010s, I have become increasingly convinced that reproducible
...
I now write most of my professional papers using Quarto and similar tools,
...
If you are interested in learning more about reproducible research, 
